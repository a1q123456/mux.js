...
var H265Stream = require('../codecs/h265.js').H265Stream;
...
VideoSegmentStream = function (track, options) {
  ...
  this.push = function (nalUnit) {
    ...
    if (nalUnit.nalUnitType === 'PPS_NUT' && !pps) {
      pps = nalUnit.data;
      track.pps = [nalUnit.data];
    }

    if (nalUnit.nalUnitType === 'SPS_NUT' && !sps) {
      sps = nalUnit.data;
      track.sps = [nalUnit.data];
    }

    if (nalUnit.nalUnitType === 'VPS_NUT' && !vps) {
      vps = nalUnit.data;
      track.vps = [nalUnit.data];
    }
  ...
  this.flush = function () {
    ...
    while (nalUnits.length) {
      if (nalUnits[0].nalUnitType === 'access_unit_delimiter_rbsp' || nalUnits[0].nalUnitType === 'AUD_NUT') {
        break;
      }
      nalUnits.shift();
    }
  ...
  this.getGopForFusion_ = function (nalUnit) {
    ...
    for (i = 0; i < this.gopCache_.length; i++) {
      ...
      if (!(track.pps && arrayEquals(track.pps[0], currentGopObj.pps[0])) ||
        ...
        !(track.vps && arrayEquals(track.vps[0], currentGopObj.vps[0]))) {
        continue;
      }
...
Transmuxer = function (options) {
  ...
  this.setupTsPipeline = function () {
    ...
    pipeline.h265Stream = new H265Stream();
    ...
    // Hook up CEA-608/708 caption stream
    pipeline.h265Stream.pipe(pipeline.captionStream)
      .pipe(pipeline.coalesceStream);
    ...
    pipeline.elementaryStream.on('data', function (data) {
      ...
      if (data.type === 'metadata') {
        ...
        if (videoTrack && !pipeline.videoSegmentStream) {
          ...
          // demux the streams
          if (videoTrack.codec === 'avc') {
            pipeline.timestampRolloverStream
              .pipe(pipeline.h264Stream);
          } else if (videoTrack.codec === 'hevc') {
            pipeline.timestampRolloverStream
              .pipe(pipeline.h265Stream);
          }
