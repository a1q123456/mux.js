...
H265Stream = function () {
  ...
  skipHrdParameter = function (commonInfPresentFlag, maxNumSubLayersMinus1, expGolombDecoder) {
    ...
    if (commonInfPresentFlag) {
      const nal_hrd_parameters_present_flag = expGolombDecoder.readBits(1);
      const vcl_hrd_parameters_present_flag = expGolombDecoder.readBits(1);
  ...
  readSequenceParameterSet = function (spsData, ppsData) {
    ...
    const conformance_window_flag = expGolombDecoder.skipBits(1);
    ...
    const log2_max_pic_order_cnt_lsb_minus4 = expGolombDecoder.skipUnsignedExpGolomb();
    ...
    for (i = (sps_sub_layer_ordering_info_present_flag ? 0 : sps_max_sub_layers_minus1); i <= sps_max_sub_layers_minus1; i++) {
      expGolombDecoder.readUnsignedExpGolomb(); // sps_max_dec_pic_buffering_minus1
      expGolombDecoder.readUnsignedExpGolomb(); // sps_max_num_reorder_pics
      expGolombDecoder.readUnsignedExpGolomb(); // sps_max_latency_increase_plus1
    }
    ...
    if (vui_parameters_present_flag) {
      ...
      if (vui_timing_info_present_flag) {
        ...
        if (vui_hrd_parameters_present_flag) {
          skipHrdParameter(1, sps_max_sub_layers_minus1);
        }
      }
    ...
    avgFrameRate = 0;
    if (vui_timing_info_present_flag) {
      avgFrameRate = vui_num_units_in_tick / vui_time_scale * 256;
    }
    constantFrameRate = 0;
